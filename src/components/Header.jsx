...
const Header = () => {
  ...
  const toggleNavigation = () => {
    setOpenNavigation((prev) => {
      ...
      isOpening ? disablePageScroll() : enablePageScroll();
      return isOpening;
    });
  };
  ...
  return (
    ...
    >
      <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
        <a className="block w-[12rem] xl:mr-8" href="#hero" aria-label="Brainwave Home">
          ...
          <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
            {navigation.map((item) => (
              <a
                ...
                } ${item.url === hash ? "lg:text-n-1 z-2" : "lg:text-n-1/50"}`}
              >
                {item.title}
              </a>
            ))}
